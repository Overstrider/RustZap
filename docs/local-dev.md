...
Use `Authorization: Bearer dev_project_key` for project routes and `Authorization: Bearer dev_admin_key` for admin/debug routes.
...
```bash
curl -H 'Authorization: Bearer dev_admin_key' http://<LAN_IP>:8167/debug/kafka
curl -H 'Authorization: Bearer dev_admin_key' http://<LAN_IP>:8167/debug/kafka/deadletters
curl -X POST -H 'Authorization: Bearer dev_admin_key' http://<LAN_IP>:8167/debug/kafka/deadletters/<deadletter_id>/replay
```
...
```bash
curl -X POST \
  -H 'Authorization: Bearer dev_project_key' \
  -H 'content-type: application/json' \
  http://<LAN_IP>:8167/v1/dev/projects/tetoz/companies/company_dev/simulate/inbound-text \
...
```bash
curl -H 'Authorization: Bearer dev_project_key' \
  'http://<LAN_IP>:8167/v1/projects/tetoz/companies/company_dev/conversations/conv_dev/messages?after_seq=0'
```
